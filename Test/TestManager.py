...
class TestManager:
    ...
    def start(self):
        for classToTest in self.subclasses:
            ...
            subclassStats = {"method_name": classToTest.__name__,
                             "passed": passed,
                             "n_passed": n_passed,
                             "tests": tests}
    ...
    def testClass(self, classToTest):
        testsStats = []
        success = True
        tests = [nom for nom, attribute in classToTest.__dict__.items() if
                 callable(attribute) and not nom.startswith("__")]
        n_passed = 0
        for test in tests:
